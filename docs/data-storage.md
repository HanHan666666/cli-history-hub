...
Claude Code History Viewer 不使用数据库，直接读写文件系统。数据由三层组成：JSONL 会话文件（Claude Code 原生生成）、sidecar 元数据文件（本项目自建）、内存缓存。
