...
Claude Code History Viewer 是一个本地 Web 应用，用于浏览和管理 Claude Code CLI 产生的对话历史。采用 Node.js + Express 后端 + 原生 JS 前端的轻量架构，无数据库依赖。

## 关联功能

- [数据存储](data-storage.md) - 数据层的详细设计
- [API 参考](api-reference.md) - 7 个后端 API 端点的完整文档
...
| 数据存储 | 文件系统 | JSONL + JSON sidecar，无数据库 |
| 样式 | 原生 CSS | 单文件 style.css，暗色主题 |

## 项目文件结构

```
claude-history-viewer/
  server.js                   # 后端：Express 服务器 + 7 个 API + 数据解析逻辑
